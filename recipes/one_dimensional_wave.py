...
class OneDimensionalWave(Bobby1D):
    def elemental(self, el):
        ...
        func_u_chi = lambda chi: self.N[0](chi)*u_local[0] + self.N[1](chi)*u_local[1]

        local_rhs[0] += integral(self.dN[0], func_u_chi)*dxdchi
        local_rhs[1] += integral(self.dN[1], func_u_chi)*dxdchi

        func_residual_chi = lambda chi : (self.dN[0](chi)*u_local[0] + self.dN[1](chi)*u_local[1]) 
        #+ dudt_local[0]*self.N[0](chi) + + dudt_local[1]*self.N[1](chi)
        func_tau_chi = lambda chi: 1.0/np.sqrt((4.0)*dchidx**2)
    ...
    def plot(self):
        ...
        if self.step_count > 0:
            del plt.gca().lines[1]
            del plt.gca().lines[0]
        plt.title("t = %.2f cfl = %.3f"%(self.t, self.cfl))
        plt.plot(self.x, self.u, "r--")
        plt.plot(self.x, np.sin(self.x - self.t - self.dt), "g-")
        plt.pause(0.0001)
        plt.ioff()
...
if __name__ == "__main__":
    ...
    x = np.linspace(-np.pi, np.pi, n*2-1)
    u = np.sin(x)
    tf = 2*np.pi*40
    cfl = 0.5
    adv = OneDimensionalWave(x, u, tf=tf, cfl=cfl, periodic=True, step_count_max = 100000)
    plt.figure(1)
